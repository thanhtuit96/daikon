...
public class RemoveSubStringRedundancies {
    ...
    public static List<Invariant> getRedundantInvariants(List<Invariant> invariants) {
        List<SimpleEntry<String, String>> pairs = getPairsFromInvariants(invariants);

        Map<String, Set<String>> filteredMap = removeRedundanciesFromMap(pairs);

        ///////////////////////////////////////////////

        List<SimpleEntry<String,String>> filteredPairs = filteredMap.entrySet().stream()
                .flatMap(entry -> entry.getValue().stream()
                        .map(value -> new SimpleEntry<>(entry.getKey(), value)))
        ...
        for(Invariant inv: invariants){
            SimpleEntry<String, String> invariantPair = getPairFromSingleInvariant(inv);
    ...
    private static List<SimpleEntry<String, String>> getPairsFromInvariants(List<Invariant> invariants){
        List<SimpleEntry<String, String>> res = new ArrayList<>();

        for(Invariant inv: invariants){
            SimpleEntry<String, String> pair = getPairFromSingleInvariant(inv);
            res.add(pair);
        }
    ...
    private static SimpleEntry<String, String> getPairFromSingleInvariant(Invariant invariant) {

        List<String> variables = getVariablesSorted(
                Arrays.asList(invariant.varNames().replace("(", "").replace(")", "").split(",")),
                invariant.toString()
                );
        return new SimpleEntry<>(variables.get(0).trim(), variables.get(1).trim());
    ...
    private static Map<String, Set<String>> removeRedundanciesFromMap(List<SimpleEntry<String,String>> input){
