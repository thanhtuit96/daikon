...

{
  ...
  public String format_using(@GuardSatisfied SeqSeqFloatGreaterEqual this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "isLexGTE(" + var1().name() + ", " + var2().name() + ")";
    }
  ...
  @Override
  public @Nullable @NonPrototype SeqSeqFloatGreaterEqual merge(
      @Prototype SeqSeqFloatGreaterEqual this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {

    SeqSeqFloatGreaterEqual result =
        (SeqSeqFloatGreaterEqual) super.merge(invs, parent_ppt);

    if (result == null) {
      return null;
    }

    result.v1ElementsMinValue = Double.MAX_VALUE;
    result.v2ElementsMaxValue = Double.MIN_VALUE;

    for (Invariant inv : invs) {
      SeqSeqFloatGreaterEqual ssgte = (SeqSeqFloatGreaterEqual) inv;

      result.v1ElementsMinValue =
          Math.min(result.v1ElementsMinValue, ssgte.v1ElementsMinValue);

      result.v2ElementsMaxValue =
          Math.max(result.v2ElementsMaxValue, ssgte.v2ElementsMaxValue);
    }
