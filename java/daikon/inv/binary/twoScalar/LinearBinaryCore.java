...
// There is no more need for swap because the equation is in standard form.
  ...
  /**
   * Returns true if the invariant is currently active. We become active after MINPAIRS
...
//                        + y + " != " + a + "*" + x + "+" + b);
  ...
  public static String formatTerm(double coeff, @Nullable String varname, boolean first) {
    ...
    String coeff_string =
        (ncoeff == (int)ncoeff) ? Integer.toString(Math.abs((int)ncoeff)) :  Double.toString(Math.abs(ncoeff));
    if (varname == null) {
