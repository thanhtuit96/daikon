...
public final class IntLessThan extends TwoScalar {
  ...
  /** Returns true if the specified var types are valid for IntLessThan. */
  @Override
  ...
  public String format_using(@GuardSatisfied IntLessThan this, OutputFormat format) {
    ...
    if (format == OutputFormat.POSTMAN) {
      return "pm.expect(" + getPostmanVariableName(var1().name()) + ").to.be.lte("
          + getPostmanVariableName(var2().name()) + ")";
    }
    if (format == OutputFormat.DSL) {
      return "lt(" + var1().name() + ", " + var2().name() + ")";
    }
  ...
  public Invariant merge(List<Invariant> invs, PptSlice parent_ppt) {
    ...
    IntLessThan result = (IntLessThan) super.merge(invs, parent_ppt);
    if (result == null) {
      return null;
    }

    result.v1MaxValue = Long.MIN_VALUE;
    result.v2MinValue = Long.MAX_VALUE;

    for (Invariant inv : invs) {
      IntLessThan other = (IntLessThan) inv;
