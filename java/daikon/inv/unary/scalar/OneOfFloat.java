...
public final class OneOfFloat extends SingleFloat implements OneOf {
  ...
  public String format_dsl(@GuardSatisfied OneOfFloat this) {
    String varname = var().name();
    if (num_elts == 1) {
      return "eq(" + varname + ", " + (Double.isNaN(elts[0]) ? "Double.NaN" : String.valueOf(elts[0])) + ")";
    } else {
      StringJoiner sj = new StringJoiner(", ", "oneOf(" + varname + ", [", "])");
      for (int i = 0; i < num_elts; i++) {
        sj.add((Double.isNaN(elts[i]) ? "Double.NaN" : String.valueOf(elts[i])));
      }
      return sj.toString();
