...
public final class OneOfScalar extends SingleScalar implements OneOf {
  ...
  public String format_dsl(@GuardSatisfied OneOfScalar this) {
    String varname = var().name();
    if (is_boolean()) {
      assert num_elts == 1;
      assert (elts[0] == 0) || (elts[0] == 1);
      return "eq(" + varname + ", " + ((elts[0] == 0) ? "false" : "true") + ")";
    } else if (is_hashcode()) {
      if (num_elts == 1) {
        if (elts[0] == 0) {
          return "eq(" + varname + ", null)";
        } else {
          return "ne(" + varname + ", null)";
        }
      } else if (num_elts == 2) {
        ...
        return "false";         // no elts, so can't be null or non-null
      } else {
