...
public final class EltOneOf extends SingleScalarSequence implements OneOf {
  ...
  public String format_dsl(@GuardSatisfied EltOneOf this) {

    if (num_elts == 1) {

      if (is_boolean()) {
        String expectedValue = (elts[0] == 0) ? "false": "true";
        return "every(" + var().name() + ", element => element == " + expectedValue + ")";
      } else if (is_hashcode()) {
        if (elts[0] == 0) {
          return "every(" + var().name() + ", element => element == null)";
        } else {
          return "//"; // We cannot check this in AGORA
        }
      } else {
        return "every(" + var().name() + ", element => [" + elts[0] + "].includes(element))";
      }
    } else {
      return "every(" + var().name() + ", element => [" + Arrays.toString(elts) + "].includes(element))";
    }
