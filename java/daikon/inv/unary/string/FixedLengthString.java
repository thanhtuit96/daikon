...
public class FixedLengthString extends SingleString {
    ...
    public String format_using(@GuardSatisfied FixedLengthString this, OutputFormat format) {
        ...
        if (format == OutputFormat.DSL) {
            return "eq(sizeOf(" + var().name() + ")," + length + ")";
        }
    ...
    public @NonNull NISuppressionSet get_ni_suppressions() {
     ...
     @Override
    public @Nullable @NonPrototype FixedLengthString merge(
      @Prototype FixedLengthString this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull FixedLengthString result = (FixedLengthString) super.merge(invs, parent_ppt);
    for (int i = 1; i < invs.size(); i++) {
      FixedLengthString inv = (FixedLengthString) invs.get(i);
      if (!Objects.equals(result.length, inv.length)) {
        return null;
      }
    }
    return result;
