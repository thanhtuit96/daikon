...
public class IsHourAMPM extends SingleString {
    ...
    public String format_using(@GuardSatisfied IsHourAMPM this, OutputFormat format) {
        ...
        if (format == OutputFormat.DSL) {
            return "isTimeOfDay(" + var().name() + ", \"HH:MM AM/PM\")";
        }
    ...
    @Override
    public @Nullable @NonPrototype IsHourAMPM merge(
        @Prototype IsHourAMPM this,
        List<@NonPrototype Invariant> invs,
        PptSlice parent_ppt) {
        return (IsHourAMPM) super.merge(invs, parent_ppt);
    }
}
