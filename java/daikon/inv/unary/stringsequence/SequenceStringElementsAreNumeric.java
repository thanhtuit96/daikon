...
public class SequenceStringElementsAreNumeric extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreNumeric this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isNumeric(element))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreNumeric merge(
      @Prototype SequenceStringElementsAreNumeric this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {

    @SuppressWarnings("nullness")
    @NonPrototype SequenceStringElementsAreNumeric result =
        (SequenceStringElementsAreNumeric) super.merge(invs, parent_ppt);

    result.alwaysEmpty = invs.stream()
        .map(inv -> (SequenceStringElementsAreNumeric) inv)
