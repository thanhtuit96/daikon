...
public class SequenceFixedLengthString extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceFixedLengthString this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", eq(sizeOf(element), " + length + "))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceFixedLengthString merge(
      @Prototype SequenceFixedLengthString this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {

    @SuppressWarnings("nullness")
    @NonPrototype SequenceFixedLengthString result =
        (SequenceFixedLengthString) super.merge(invs, parent_ppt);

    Integer length = null;

    for (Invariant invBase : invs) {
      SequenceFixedLengthString inv =
          (SequenceFixedLengthString) invBase;
      ...
      if (length == null) {
        length = inv.length;
      } else if (!length.equals(inv.length)) {
        return null;
      }
    }
