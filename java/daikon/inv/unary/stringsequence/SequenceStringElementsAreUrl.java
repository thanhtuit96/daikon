...
public class SequenceStringElementsAreUrl extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreUrl this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isURL(element))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreUrl merge(
      @Prototype SequenceStringElementsAreUrl this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreUrl result = (SequenceStringElementsAreUrl) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty = invs.stream()
        .map(inv -> (SequenceStringElementsAreUrl) inv)
