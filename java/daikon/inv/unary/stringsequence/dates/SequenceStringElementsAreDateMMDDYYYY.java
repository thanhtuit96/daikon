...
public class SequenceStringElementsAreDateMMDDYYYY extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreDateMMDDYYYY this, OutputFormat format) {
    ...
    if (format == OutputFormat.POSTMAN) {
      return "pm.expect(" + getPostmanVariableName(var().name()) + ".every(element => /" + regex
          + "/.test(element))).to.be.true";
    }
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isDate(element, \"MM/DD/YYYY\"))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreDateMMDDYYYY merge(
      @Prototype SequenceStringElementsAreDateMMDDYYYY this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreDateMMDDYYYY result = (SequenceStringElementsAreDateMMDDYYYY) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreDateMMDDYYYY) inv)
            .allMatch(inv -> inv.alwaysEmpty);
