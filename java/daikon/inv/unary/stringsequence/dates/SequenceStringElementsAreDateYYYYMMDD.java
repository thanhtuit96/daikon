...
public class SequenceStringElementsAreDateYYYYMMDD extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreDateYYYYMMDD this, OutputFormat format) {
    if (format == OutputFormat.DAIKON) {
      return "All the elements of " + var().name() + " are dates. Format: YYYY/MM/DD or YYYY-MM-DD or YYYY.MM.DD";
    }
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isDate(element, \"YYYY/MM/DD\"))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreDateYYYYMMDD merge(
      @Prototype SequenceStringElementsAreDateYYYYMMDD this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreDateYYYYMMDD result = (SequenceStringElementsAreDateYYYYMMDD) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreDateYYYYMMDD) inv)
