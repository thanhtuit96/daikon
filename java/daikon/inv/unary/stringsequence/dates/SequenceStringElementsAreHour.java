...
public class SequenceStringElementsAreHour extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreHour this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isTimeOfDay(element))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreHour merge(
      @Prototype SequenceStringElementsAreHour this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreHour result = (SequenceStringElementsAreHour) super.merge(invs, parent_ppt);
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreHour) inv)
