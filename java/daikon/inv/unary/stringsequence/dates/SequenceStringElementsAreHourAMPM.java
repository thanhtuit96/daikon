...
public class SequenceStringElementsAreHourAMPM extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreHourAMPM this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isTimeOfDay(element, \"HH:MM AM/PM\"))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreHourAMPM merge(
      @Prototype SequenceStringElementsAreHourAMPM this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreHourAMPM result = (SequenceStringElementsAreHourAMPM) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreHourAMPM) inv)
