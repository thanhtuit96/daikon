...
public class SequenceStringElementsAreDateDDMMYYYY extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreDateDDMMYYYY this, OutputFormat format) {
    if (format == OutputFormat.DAIKON) {
      return "All the elements of " + var().name() + " are dates. Format: DD/MM/YYYY or DD-MM-YYYY or DD.MM.YYYY";
    }
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isDate(element, \"DD/MM/YYYY\"))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreDateDDMMYYYY merge(
      @Prototype SequenceStringElementsAreDateDDMMYYYY this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreDateDDMMYYYY result = (SequenceStringElementsAreDateDDMMYYYY) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreDateDDMMYYYY) inv)
