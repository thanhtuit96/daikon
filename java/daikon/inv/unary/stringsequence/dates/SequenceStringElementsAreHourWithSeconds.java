...
public class SequenceStringElementsAreHourWithSeconds extends SingleStringSequence {
  ...
  public String format_using(@GuardSatisfied SequenceStringElementsAreHourWithSeconds this, OutputFormat format) {
    ...
    if (format == OutputFormat.DSL) {
      return "forall(" + var().name() + ", isTimeOfDay(element, \"HH:MM:SS\"))";
    }
  ...
  @Override
  public @Nullable @NonPrototype SequenceStringElementsAreHourWithSeconds merge(
      @Prototype SequenceStringElementsAreHourWithSeconds this,
      List<@NonPrototype Invariant> invs,
      PptSlice parent_ppt) {
    @SuppressWarnings("nullness") // super.merge does not return null
    @NonNull SequenceStringElementsAreHourWithSeconds result = (SequenceStringElementsAreHourWithSeconds) super.merge(invs, parent_ppt);
    
    result.alwaysEmpty =
        invs.stream()
            .map(inv -> (SequenceStringElementsAreHourWithSeconds) inv)
